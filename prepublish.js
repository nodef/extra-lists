...
// Global variables.
const ORG = 'extra-lists';
const ORIGINAL = 'Original module: [extra-lists].';
...
function pkgMinify(o) {
  ...
  readme = readme.replace(/(\.<br>)/, ', exported as `Lists`$1');
  readme = readme.replace(/(\.<br>)[\s\S]*?(\[!\[nodef\])/, `$1${EOL}${ORIGINAL}${EOL}${CDN}${EOL}<br>${EOL}${EOL}${EOL}$2`);
  readme = readme.replace(/extra-lists\.min/g, 'extra-lists');
  readme += `[unpkg]: https://unpkg.com/extra-lists.min${EOL}`;
  readme += `[jsDelivr]: https://cdn.jsdelivr.net/npm/extra-lists.min${EOL}`;
  pkg.name += '.min';
